...
namespace code_generator
{
    ...
    public:
        ...
        QString execute_build() const
        {
            ...
            std::tie(timer_handlers, key_handlers, pgn_name_msgs, msg_handler_pgn_all, on_dll_load) = util::read_def(def_file_path.toStdString());
            util::SystemTasksInfo tasks_info(timer_handlers.size(), key_handlers.size() != 0, can_sender, msg_handler_pgn_all.declared, pgn_name_msgs.size() != 0, on_dll_load.declared, serial_user);
            ...
            std::tie(timer_tasks, key_tasks, pgn_name_tasks, pgn_all_task, dll_load_task, functions, error_on_key_mapping_str) = util::get_functions(cpp_src_file_path.toStdString(), timer_handlers, key_handlers, pgn_name_msgs, key_mapping, msg_handler_pgn_all, on_dll_load, error_on_key_mapping);
            ...
            code_generator::CppFileGenerator cpp_file_generator{output_cpp_file_path,
                                                                ...
                                                                R"(:/code_templates/pgn_name_switch.txt)",
